...
model = dict(
    ...
    decode_head_st=dict(
        ...
        #sampler=dict(type='OHEMPixelSampler', thresh=0.7, min_kept=100000),
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.0, 1.25, 1.25, 1.0, 1.5, 1.5, 1.25])),
    decode_head_ts=dict(
        type='SegformerHead',
        in_channels=[64, 128, 320, 512],
        in_index=[0, 1, 2, 3],
    ...
    cross_EMA = dict(
        ...
        pseudo_class_weight=[1.51, 1.01, 1.51, 1.01, 2.01, 1.51, 1.01],
        backbone_EMA=dict(
            type='MixVisionTransformer',
            init_cfg=dict(type='Pretrained', checkpoint='/pretrained/mit_b5.pth'),
            in_channels=3,
...
data = dict(samples_per_gpu=5, workers_per_gpu=1)
